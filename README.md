...
1. Importa el archivo `spamascotas.sql` proporcionado en tu gestor de base de datos (ej. phpMyAdmin).
2. **Limpiar datos de prueba:** Para evitar conflictos con los usuarios de prueba originales, ejecuta el siguiente código SQL en la pestaña SQL de phpMyAdmin. Esto limpiará las cuentas antiguas de forma segura:
...
1. Busca en Telegram a `@BotFather` para crear un bot y obtener tu **Token**.
2. Busca a `@userinfobot` para obtener tu **Chat ID** numérico.
3. Configura tus variables de entorno:
   * Abre el archivo `.env.example`, llénalo con el Token y el Chat ID que acabas de obtener, además de las credenciales de tu base de datos local.
   * Guarda ese archivo renombrándolo a `.env` (asegúrate de que tenga el punto al inicio). El sistema leerá automáticamente las credenciales desde ahí para procesar las compras en `catalogo.php` y `dashboard.php`.
